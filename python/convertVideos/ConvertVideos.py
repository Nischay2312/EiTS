...
class VideoConverterApp:
    def __init__(self, root):
        ...
        tk.Button(root, text="Convert", command=self.start_conversion_thread, width=20).grid(row=4, column=0, padx=10, pady=20)
        tk.Button(root, text="Repeat", command=self.reset_gui, width=10).grid(row=4, column=1, padx=10, pady=20)
        tk.Button(root, text="Exit", command=root.quit, width=10).grid(row=4, column=2, padx=10, pady=20)

        tk.Label(root, text="CONSOLE OUTPUT", font=("Arial", 12, "bold"), anchor="w").grid(row=5, column=0, columnspan=3, padx=10, pady=5, sticky="w")

        self.console = tk.Text(root, height=10, width=50)
        self.console.grid(row=7, column=0, columnspan=3, padx=10, pady=10, sticky='nsew')

        self.console_scrollbar = tk.Scrollbar(root, command=self.console.yview)
        self.console_scrollbar.grid(row=7, column=3, sticky='nsew')
        self.console['yscrollcommand'] = self.console_scrollbar.set
                
        tk.Button(root, text="Clear Console", command=self.clear_console, width=15).grid(row=8, column=2, padx=10, pady=5)
        
        self.progress = ttk.Progressbar(root, orient=tk.HORIZONTAL, length=300, mode='determinate')
        self.progress.grid(row=6, column=0, columnspan=3, padx=(10, 0), pady=20, sticky= "ew")
